...
contract WithdrawalChallenge is Test {
    ...
    function test_withdrawal() public checkSolvedByPlayer {
       ...
       uint256 WITHDRAWAL_TOTAL = 999000e18;
        ...
        Message[] memory msgs = abi.decode(data, (Message[]));
        
        uint256 last_withdrawal_tstmp = 0;

        for(uint i = 0; i < msgs.length; i++){
            bytes32 id;
            uint256 tstmp;
            bytes memory msgdata;
            (id, tstmp, msgdata) = abi.decode(msgs[i].data, (bytes32, uint256, bytes));
            if(last_withdrawal_tstmp < tstmp)
                last_withdrawal_tstmp = tstmp;
        }

        vm.warp(last_withdrawal_tstmp + 7 days);

        

        for(uint i = 0; i < msgs.length; i++){
            Topics memory topics = abi.decode(abi.encodePacked(msgs[i].topics),(Topics));
            bytes32 id;
            uint256 tstmp;
            bytes memory msgdata;
            (id, tstmp, msgdata) = abi.decode(msgs[i].data, (bytes32, uint256, bytes));
            ...
            /*
            console.log("timestamp",tstmp);
            console.logBytes(msgdata);
            console.log("caller",topics.caller);
            console.log("target", topics.target);
            */
        ...
        token.transfer(address(l1TokenBridge), WITHDRAWAL_TOTAL);
